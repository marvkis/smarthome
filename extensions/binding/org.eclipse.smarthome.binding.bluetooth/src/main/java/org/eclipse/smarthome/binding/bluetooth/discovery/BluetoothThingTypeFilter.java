...
/**
 * The {@link BluetoothThingTypeFilter} handles the BLE filter which is used to select a thing type from the device
 * attributes.
 *
 * @author Chris Jackson - Initial Contribution
 *
 */
public class BluetoothThingTypeFilter {
    private final static Logger logger = LoggerFactory.getLogger(BluetoothThingTypeFilter.class);
    ...
    protected static void setThingTypeRegistry(ThingTypeRegistry thingTypeRegistry) {
        BluetoothThingTypeFilter.thingTypeRegistry = thingTypeRegistry;
    }

    protected static void unsetThingTypeRegistry(ThingTypeRegistry thingTypeRegistry) {
        BluetoothThingTypeFilter.thingTypeRegistry = null;
    }
    ...
    private static void initialiseThingTypeList() {
        ...
        for (ThingType thingType : thingTypes) {
            ...
            if (BluetoothBindingConstants.BINDING_ID.equals(thingType.getBindingId()) == false) {
                continue;
            }
    ...
    public static boolean checkFilters(ThingTypeUID uid, BluetoothDevice device, String[] filterArray) {
     ...
     */
    public static ThingTypeUID findThingType(BluetoothDevice device) {
        ...
        for (ThingType thingType : thingTypes) {
            ...
            if (thingProperties.get(BluetoothBindingConstants.XMLPROPERTY_BLE_FILTER) == null) {
                logger.debug("BLE thing {} has no filter so can't be discovered!", thingType.getUID());
                continue;
            }

            String[] filterArray = thingProperties.get(BluetoothBindingConstants.XMLPROPERTY_BLE_FILTER).split(",");
